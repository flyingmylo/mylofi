---
title: "面经记录"
summary: '记录疫情解封后的一次面试题'
date: 2022-07-13T21:27:03+08:00
tags: ['笔记']
showTags: true
slug: interview
draft: false
...
- `filter` 怎么实现，使用场景
- 插槽的实现原理，如何传值
- `v-for` 中 `key` 的作用，延伸到 `diff` 实现，不同位置的比较
- `vnode` 概念
...
- 伪元素和伪类的区别，分别有哪些
- `visibility: hidden` 和 `opacity` 是否触发点击事件
...
- 实例对象是否是 `Function` ，`instanceof Function` 结果
- `promise` 执行顺序，事件循环
- `es5` 模拟 `class` 的实现
